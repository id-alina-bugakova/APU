...
*  k_pid_un1 = -4.7619, k_pid_yn = 0.9048, u - разность между желаемыми оборотами и реальными).
...
typedef struct
{
    ...
    double p;                           ///< Пропорциональная составляющая регулятора
    double i;                           ///< Интегральная составляющая регулятора
    double d;                           ///< Дифференциальная составляющая регулятора
...
*  Рабочие обороты ротора 39000 об./мин. (малые обороты 10000 об./мин.). В аварийном режиме рабочие
*  обороты составляют 32000 об./мин.
*  Температура выхлопных газов моделируется также при помощи численного интегрирования методом 
*  трапеций (с коэффициентом k_EGT) и учитывает охлаждение вентилятором.
*  На роторе находятся 2 датчика частоты вращения и 4 датчика температуры выхлопных газов, 
*  разделенные на 2 канала.
*/
typedef struct
{
...
*  без редукции). При подаче воздуха в пневмосистему отбираемый момент компрессора увеличивается в
*  k_inc раз.
*  Температура вычисляется по формуле T3 [К] = T2 [К] * (k_T1 * P3/P2 + k_T2).
*  Электропитание отсутствует, компресор приводится в движение от ротора через редуктор.
*/
typedef struct
{
...
/* @brief  Генератор
* 
*  Может быть включен на оборотах более N_turnon (около 75% полных оборотов, 30000 об./мин).
*  Потребляет постоянный момент M (15) Н*м и выдает постоянную стабильную частоту вращения 
*  12000 об./мин, которая считывается датчиком NGC.
*/
typedef struct
{
...
*
*  Определяет подачу топлива в газогенератор. Потребляет постоянный момент мощности 1 Н*м, когда 
*  подача топлива включена. 
*  Включает клапан отсечки топлива и датчик давления топлива (дискретный - есть/нет).
*/
typedef struct
{
    bool power;                         ///< Признак питания (0 - нет, 1 - есть)
...
*  @param Pneumatic_system* psys : указатель на пневмосистему для получения T2 и P2
*/
...
*  @param Compressor* comp : указатель на компрессор для вычисления параметров воздуха
*/
