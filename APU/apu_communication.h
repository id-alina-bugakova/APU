/* @file   apu_communication.h
*  @brief  Описание структур, используемых для передачи данных в системе
*
*  @detail Файл содержит информацию о структурах, используемых для информационного обмена внутри
*  системы и со внешними системами.
*
*  @author Бугакова А.А.
*/
...
typedef struct {
    ...
    * 
    *  Для блокировки и/или выключения генератора при наличии наземного питания.
    */
    bool ext_power_avail;

    bool gen_switch;                    ///< Запрос на вкл./выкл. генератора

    /* @brief Признак подключения генератора к шине СЭС
    * 
    *  Подтверждение для ВСУ, что генератор подключен кшине СЭС. Генератор не должен работать 
    *  вхолостую.
    */
    bool apb_stat;                      
} EPS_message;
...
/* @brief Структура отправки данных СКВ
*
...
/* @brief Структура автоматичсеких действий
* 
*  Действия выполняются драйвером после отработки контроллера. 
*/
typedef struct {
...
/* @brief Структура ручных действий
*
*  Действия выполняются драйвером перед отработкой контроллера.
*/
typedef struct {
...
*  
*  Информация о физических событиях
*/
typedef struct {
...
/* @brief Функция инициализации структуры сообщений
* 
*  @param Messages* msgs : инициализируемая структура
*/
void init_messages(Messages* msgs);

/* @brief Функция инициализации структуры ответов
*
*  @param Responses* rsps : инициализируемая структура
*/
void init_responses(Responses* rsps);

/* @brief Функция инициализации структуры автоматических действий
*
*  @param Actions* acts : инициализируемая структура
*/
void init_actions(Actions* acts);

/* @brief Функция инициализации структуры ручных действий
*
*  @param Actions_manual* actm : инициализируемая структура
*/
void init_actions_manual(Actions_manual* actm);

/* @brief Функция инициализации структуры данных о системе
*
*  @param Data* data : инициализируемая структура
*/
void init_data(Data* data);

/* @brief Функция инициализации структуры физических событий
*
*  @param Physical* phys : инициализируемая структура
*/
void init_physical(Physical* phys);


/* @brief Функция сброса структуры сообщений
*
*  @param Messages* msgs : сбрасываемая структура
*/
void reset_messages(Messages* msgs);


/* @brief Функция копирования структуры ответов
*
*  @param Responses* original : копируемая структура
*  @param Responses* copy : структура, в которую копируем
*/
